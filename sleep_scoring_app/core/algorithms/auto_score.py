...
Example usage:
    >>> from sleep_scoring_app.core.algorithms import auto_score_activity_epoch_files
...
def _calculate_metrics(
    ...
    choi_onset = int(period_choi.iloc[0]) if period_choi is not None and len(period_choi) > 0 else None
    choi_offset = int(period_choi.iloc[-1]) if period_choi is not None and len(period_choi) > 0 else None
    total_choi_counts = int(period_choi.sum()) if period_choi is not None else None
