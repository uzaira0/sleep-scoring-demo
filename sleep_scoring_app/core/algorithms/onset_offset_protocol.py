"""
Onset/offset detection protocol for dependency injection.

This protocol defines the interface that all onset/offset rule implementations must follow.
Enables swappable rules without changing core application logic.

Critical distinction:
- Sleep scoring algorithms (Sadeh, Cole-Kripke) classify each epoch as sleep/wake
- Onset/offset rules find sleep period boundaries within classified data
"""
...
@runtime_checkable
class OnsetOffsetRule(Protocol):
    """
    Protocol for sleep onset/offset detection rules.

    All rule implementations (Consecutive N Minutes, Tudor-Locke, etc.) must implement this interface.

    The rule operates on already-classified sleep/wake data to identify the precise
    boundaries of sleep periods.
    """
    ...
    def apply_rules(
        ...
        """
        Apply onset/offset detection rules to find sleep period boundaries.
