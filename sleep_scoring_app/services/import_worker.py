...
class ImportWorkerObject(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            # Connect service signals to worker signals for forwarding
            self.import_service.progress_updated.connect(self.progress_updated.emit)
            self.import_service.nonwear_progress_updated.connect(self.nonwear_progress_updated.emit)
            self.import_service.import_completed.connect(self.import_completed.emit)
            ...
            if isinstance(self.directory_or_files, list):
                ...
                self.import_service.import_directory(
                    self.directory_or_files,
                    self.skip_rows,
                    self.force_reimport,
                    progress_callback=lambda p: self.progress_updated.emit(p),
