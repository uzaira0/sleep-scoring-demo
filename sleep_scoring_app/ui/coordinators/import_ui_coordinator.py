...
class ImportUICoordinator:
    ...
    def browse_data_folder(self) -> None:
        ...
        start_dir = self.parent.config_manager.config.data_folder or str(Path.home())
        directory = QFileDialog.getExistingDirectory(self.parent, "Select Data Folder", start_dir)
    ...
    def browse_activity_files(self) -> None:
        ...
        start_dir = self.parent.config_manager.config.import_activity_directory or str(Path.home())
        ...
        if files:
            try:
                ...
                self.parent.config_manager.config.import_activity_directory = str(Path(files[0]).parent)
    ...
    def browse_nonwear_files(self) -> None:
        ...
        start_dir = self.parent.config_manager.config.import_nonwear_directory or str(Path.home())
        files, _ = QFileDialog.getOpenFileNames(
            self.parent,
        ...
        if files:
            try:
                ...
                # Save directory of first file to config for next browse
                self.parent.config_manager.config.import_nonwear_directory = str(Path(files[0]).parent)
                self.parent.config_manager.save_config()
            except AttributeError as e:
                logger.warning("Parent window missing required attributes: %s", e)
    ...
    def start_activity_import(self) -> None:
        ...
        custom_columns = None
        if self.parent.config_manager.config.device_preset == "generic_csv":
        ...
        skip_rows = self.parent.config_manager.config.skip_rows
    ...
    def start_nonwear_import(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self.parent, "Import Error", f"Failed to start nonwear import: {e}")
