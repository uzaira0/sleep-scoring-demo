...
from sleep_scoring_app.core.constants import TableColumn, TooltipText
...
class TestTableCreationWithAlgorithmName:
    ...
    def test_create_table_with_sadeh_algorithm(self, qapp) -> None:
        """Test creating table with Sadeh algorithm name."""
        table_container = create_marker_data_table("Test Table", sleep_algorithm_name="Sadeh")

        table = table_container.table_widget
        header_item = table.horizontalHeaderItem(3)

        assert header_item is not None
        assert header_item.text() == "Sadeh"

    def test_create_table_with_cole_kripke_algorithm(self, qapp) -> None:
        """Test creating table with Cole-Kripke algorithm name."""
        table_container = create_marker_data_table("Test Table", sleep_algorithm_name="Cole-Kripke")

        table = table_container.table_widget
        header_item = table.horizontalHeaderItem(3)

        assert header_item is not None
        assert header_item.text() == "Cole-Kripke"

    def test_create_table_stores_algorithm_name(self, qapp) -> None:
        """Test that algorithm name is stored on the container."""
        table_container = create_marker_data_table("Test Table", sleep_algorithm_name="Sadeh")

        assert hasattr(table_container, "sleep_algorithm_name")
        assert table_container.sleep_algorithm_name == "Sadeh"
    ...
    def test_table_has_correct_column_count(self, qapp) -> None:
    ...
    def test_algorithm_tooltip_is_set_correctly(self, qapp) -> None:
        ...
        assert header_item is not None
        assert "Cole-Kripke" in header_item.toolTip()
...
class TestUpdateTableSleepAlgorithmHeader:
    ...
    def test_update_header_to_cole_kripke(self, qapp) -> None:
        """Test updating header to Cole-Kripke algorithm."""
        table_container = create_marker_data_table("Test Table", sleep_algorithm_name="Sadeh")

        update_table_sleep_algorithm_header(table_container, "Cole-Kripke")
    ...
    def test_update_header_updates_tooltip(self, qapp) -> None:
        """Test that updating header also updates tooltip."""
        table_container = create_marker_data_table("Test Table", sleep_algorithm_name="Sadeh")

        update_table_sleep_algorithm_header(table_container, "Van Hees 2015")

        table = table_container.table_widget
        header_item = table.horizontalHeaderItem(3)

        assert "Van Hees 2015" in header_item.toolTip()

    def test_update_header_stores_new_algorithm_name(self, qapp) -> None:
        """Test that updating header updates stored algorithm name."""
        table_container = create_marker_data_table("Test Table", sleep_algorithm_name="Sadeh")

        update_table_sleep_algorithm_header(table_container, "Cole-Kripke")

        assert table_container.sleep_algorithm_name == "Cole-Kripke"

    def test_update_header_with_invalid_container_logs_warning(self, qapp) -> None:
        """Test that updating header with invalid container logs warning."""
        invalid_container = QWidget()

        # Should not raise an exception
        update_table_sleep_algorithm_header(invalid_container, "Sadeh")

    def test_update_header_preserves_other_columns(self, qapp) -> None:
...
class TestUpdateMarkerTableWithSleepScore:
    ...
    def test_update_table_with_sleep_score_key(self, qapp) -> None:
    ...
    def test_update_table_without_sleep_score_shows_placeholder(self, qapp) -> None:
...
class TestMarkerTableManagerIntegration:
    ...
    def test_get_current_sleep_algorithm_name_returns_string(self) -> None:
        ...
        # Create mock parent with minimal required attributes
        # Set plot_widget to None to trigger the default fallback
        mock_parent = MagicMock()
        mock_parent.config_manager = None
        mock_parent.plot_widget = None

        manager = MarkerTableManager(mock_parent)
        result = manager.get_current_sleep_algorithm_name()

        assert isinstance(result, str)
        assert len(result) > 0

    def test_get_current_sleep_algorithm_name_default_value(self) -> None:
        """Test that get_current_sleep_algorithm_name returns default when no config."""
        from sleep_scoring_app.ui.marker_table import DEFAULT_SLEEP_ALGORITHM_NAME, MarkerTableManager

        # Set both config_manager and plot_widget to None to ensure default fallback
        mock_parent = MagicMock()
        mock_parent.config_manager = None
        mock_parent.plot_widget = None
        ...
        assert result == DEFAULT_SLEEP_ALGORITHM_NAME

    def test_get_current_sleep_algorithm_name_from_config(self) -> None:
        """Test that get_current_sleep_algorithm_name reads from config."""
        from sleep_scoring_app.ui.marker_table import MarkerTableManager

        mock_parent = MagicMock()
        mock_config = MagicMock()
        mock_config.sleep_algorithm_id = "cole_kripke_1992_actilife"
        mock_parent.config_manager.config = mock_config

        manager = MarkerTableManager(mock_parent)
        result = manager.get_current_sleep_algorithm_name()

        # Should return the display name from factory
        assert result == "Cole-Kripke (1992) ActiLife"

    def test_update_table_headers_for_algorithm_caches_name(self) -> None:
        """Test that update_table_headers_for_algorithm caches the algorithm name."""
        from sleep_scoring_app.ui.marker_table import MarkerTableManager

        mock_parent = MagicMock()
        mock_parent.config_manager = None
        mock_parent.onset_table = None
        mock_parent.offset_table = None
    ...
    def test_update_table_headers_skips_if_unchanged(self) -> None:
        ...
        # Pre-set the cache to the default value
        manager._cached_algorithm_name = DEFAULT_SLEEP_ALGORITHM_NAME
